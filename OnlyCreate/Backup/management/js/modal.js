...
function setModal() 
{
    ...
    function onComplete() 
    {
        ...
        // 完了ボタンによるメイン画面への入力データ渡し（ディスタッフ流用）
        ...
        // 完了ボタンによるメイン画面への入力データ渡し（ディスタッフ流用）
        $("input#complete_continue").click(function()
        {
            setInputDataAndCloseContinue();
            return false;
        });
    }
}
...
// 値をセットして戻る（連続入力専用）
function setInputDataAndCloseContinue()
{
    // 値をセットしたブロック数分(連続入力数分)ループさせる
    $(".gyodata").each(function()
    {
        
        // 戻し用のエリアID
        var areaid =  "area" + $(this).children(".areaid").val();
        ...
        // 入力パラメータを配列で取得する
        var params = $(this).children(".params").map(function()
        {
            if (($(this).attr("name") != "areaid") && ($(this).attr("name") != "callback"))
            {
                return $(this).val();
...
// 値をクリアして戻る
function setNullAndClose() 
{
    var dialognum = $("#dialognum").val();
    var nulltext = "";
    
    // textboxならval、それ以外ならtextを使用する
    setElementValue("#keyword"+dialognum, nulltext);
    setElementValue("#retword_a"+dialognum, nulltext);
    setElementValue("#retword_b"+dialognum, nulltext);
    setElementValue("#retword_c"+dialognum, nulltext);
    setElementValue("#retword_d"+dialognum, nulltext);
    setElementValue("#retword_e"+dialognum, nulltext);
    $("div#modal").fadeOut(250);
}
